...
def run(seed, opponent="demo", swapped=False, pressure=1, limit=1300, replay=None, url=None):
    game = Game(seed, pressure)
    participant = http_policy(url) if url else Agent().decide
    other = demo_policy() if opponent == "demo" else Agent().decide if opponent == "self" else lambda _: empty_response()
    ...
    with ThreadPoolExecutor(max_workers=2) as pool:
        while not game.finished and game.round <= limit:
            ...
            frame = {"round": game.round, "observations": observations, "responses": responses}
            game.step(responses)
            if replay:
...
def main():
    ...
    parser.add_argument("--opponent", choices=("demo", "self", "idle"), default="demo")
    ...
    report = {"scope": "local experimental evidence, not official validation", "python": platform.python_version(),
              ...
              "assumptions": "docs/IMPLEMENTATION.md A01-A09", "hashes": source_hashes(), "games": []}
    try:
